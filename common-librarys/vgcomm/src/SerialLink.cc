...
, _port(NULL), _stopp(false), _reqReset(false)
{
...
bool SerialLink::_isBootloader()
{
    ...
    foreach (const QSerialPortInfo &info, portList)
    {
        ...
        if ( info.portName().trimmed() == name &&
             (     description.contains("bootloader")
                || description.contains("px4 bl")
                || description.contains("px4 fmu v1.6")) )
            return true;
    }
    // Not found
    return false;
...
{
    QString msg(tr("Error on link %1. %2"));
    emit communicationError(tr("Serial link error"), msg.arg(devName()).arg(errorMsg));//"串口通信错误"
}
//--------------------------------------------------------------------------
//-- SerialCommand

SerialCommand::SerialCommand(QObject *p) : LinkCommand(p), m_baud(57600)
, m_dataBits(8), m_flowControl(QSerialPort::NoFlowControl), m_stopBits(1)
, m_parity(QSerialPort::NoParity), m_usbDirect(false)
{
}
...
LinkInterface *SerialCommand::CreateLink()
{
