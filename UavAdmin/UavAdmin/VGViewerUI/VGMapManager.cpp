...
const int s_scaleLengths[] = { 5, 10, 20, 50, 100
, 200, 500, 1000, 2000, 5000
, 10000, 20000, 50000, 100000
, 200000, 500000, 1000000, 2000000 };

static QGeoCoordinate coordinateFromString(const QString &str)
{
    QStringList ls = str.split(str, QString::SkipEmptyParts);
    if (ls.count() >= 2)
    {
        return QGeoCoordinate(ls.first().toDouble(), ls.at(1).toDouble());
    }
...
, m_bShowSR(true), m_mapCenter(40.12121, 119.12121)
...
void VGMapManager::_readConfig()
{
	if (QSettings *settings = qvgApp->GetSettings())
	{
		...
		QGeoCoordinate coor = coordinateFromString(settings->value("center").toString());
		if (coor.isValid())
			setMapCenter(coor);
		m_bMapChanged = false;
		settings->endGroup();
...
void VGMapManager::_writeConfig()
{
	if (QSettings *settings = qvgApp->GetSettings())
	{
		...
		settings->setValue("center", m_mapCenter.toString(QGeoCoordinate::Degrees));
		settings->endGroup();
	}
}
