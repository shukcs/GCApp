...
{
    return QFontMetricsF(f).width(str);
}
...
bool VGMainPage::GetScreenHoriz() const
{
    return m_bScrHoriz;
}

void VGMainPage::SetScreenHoriz(bool sh)
{
    if (m_bScrHoriz == sh)
        return;
#if __android__
    jint orient = sh ? 0 : 1;    //0:横屏, 1:竖屏
    QAndroidJniObject activity = QtAndroid::androidActivity();
    activity.callMethod<void>("setRequestedOrientation", "(I)V", orient);
    QAndroidJniEnvironment env;
    if (env->ExceptionCheck())
    {
        qDebug() << "exception when setRequestedOrientation";
        env->ExceptionClear();
        return;
    }
    m_bScrHoriz = sh;
    emit scrHorizChanged(sh);
#endif
}
