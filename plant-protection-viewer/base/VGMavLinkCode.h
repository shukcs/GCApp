...
namespace VGMavLinkCode {
    ...
    void EncodeQXSdkAccount(mavlink_message_t &msg, const QString &key, const QString &secret,const QString &id, const QString &type, MAVLinkProtocol *p = NULL, int ch = 0);
    ...
    int DecodeSupport(const mavlink_message_t &msg, QGeoCoordinate &cEtr, QGeoCoordinate &cRet);
    bool DecodeGlobalPos(const mavlink_message_t &msg, QGeoCoordinate &pos, double &vHor, double &vVer, double &alt);
    bool DecodeGpsSatlate(const mavlink_message_t &msg, QGeoCoordinate &pos, int &nNumb, uint8_t &fix, double &pre, uint16_t& cog);
    ...
    bool DecodeQXSDKAccount(const mavlink_message_t &msg, QString &key, QString &secret, QString &id, QString &type);
}
