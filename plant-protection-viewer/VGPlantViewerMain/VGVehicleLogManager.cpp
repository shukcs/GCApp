...
void VGVehicleLogManager::sltRequestLogData(int logId, long size, long time_utc)
{
    ...
    for(int i = 0; i < m_lstLogDownloadData.size(); i++)
    {
        ...
        if(logData)
        {
            if(logData->_vehicleID == m_vehicle->sysID() && logData->_logID == logId)
            {
                return;
            }
        }
    ...
    LogDownloadData* newLogDownload = new LogDownloadData(m_vehicle->sysID(), m_vehicle->firewareType(), logId, size, time_utc);
...
void VGVehicleLogManager::sltRequestLogList()
{
    if(m_vehicle)
    {
        _requestLogList(m_vehicle->sysID(), 0, 49);
    }
}
