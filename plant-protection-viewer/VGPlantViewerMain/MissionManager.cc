...
void MissionManager::_writeMissionCount(void)
{
    if (m_dedicatedLink = m_vehicle->priorityLink())
    {
        ...
        VGMavLinkCode::EncodeMissionCount(message, m_writeMissionItems.count(), (MAV_MISSION_TYPE)m_typeMissionWrite,
            qvgApp->toolbox()->mavlinkProtocol(), m_dedicatedLink->getMavlinkChannel(), MAV_COMP_ID_MISSIONPLANNER, m_vehicle->sysID());
        m_vehicle->sendMessage(message);
...
void MissionManager::writeArduPilotGuidedMissionItem(const QGeoCoordinate& gotoCoord, bool altChangeOnly)
{
    ...
    missionItem.target_system =     m_vehicle->sysID();
    ...
    m_vehicle->sendMessage(messageOut);
...
void MissionManager::_requestList()
{
    ...
    request.target_system = m_vehicle->sysID();
...
void MissionManager::_ackTimeout(void)
{
    ...
    case AckNone:
        ...
        _sendError(InternalError, tr("Mission miss link"));//"任务通讯失败"
        break;
    case AckMissionCount:
        // MISSION_COUNT message expected
        if (m_retryCount > _maxRetryCount) {
            _sendError(VehicleError, tr("download fail,try too much"));//"下载航线失败, 重试超限"
    ...
    case AckMissionItem:
        ...
        if (m_retryCount > _maxRetryCount) {
            _sendError(VehicleError, tr("download mission fail"));//"下载航线失败"
            _finishTransaction(false);
        } else {
            m_retryCount++;
    ...
    case AckMissionRequest:
        ...
        if (m_itemIndicesToWrite.count() == 0)
        {
            ...
            _sendError(VehicleError, tr("upload mission fail"));//上传航线失败"
            _finishTransaction(false);
        }
        else if (m_itemIndicesToWrite[0] == 0)
        {
            ...
            if (m_retryCount > _maxRetryCount)
            {
                _sendError(VehicleError, tr("try upload too much"));//"尝试上传次数过多"
                _finishTransaction(false);
            }
            else
            {
        ...
        else
        {
            ...
            _sendError(AckTimeoutError, tr("not read all mission items"));//"设备没有读取所有任务点"
    ...
    case AckMissionClearAll:
        ...
        if (m_retryCount > _maxRetryCount) {
            _sendError(VehicleError, tr("try delete mission too much"));//"删除任务次数超限"
            _finishTransaction(false);
        } else {
            m_retryCount++;
    ...
    case AckGuidedItem:
        // MISSION_REQUEST is expected, or MISSION_ACK to end sequence
    default:
        _sendError(AckTimeoutError, tr("upload mission fail,no respond from device"));//"上传航线失败，设备没有回应任务点"
        m_expectedAck = AckNone;
        _finishTransaction(false);
    }
}
...
void MissionManager::_readTransactionComplete(void)
{
    ...
    missionAck.target_system =      m_vehicle->sysID();
...
void MissionManager::_requestNextMissionItem(void)
{
    ...
    VGMavLinkCode::EncodeReadItem(message, m_itemIndicesToRead[0], m_vehicle->supportsMissionItemInt(), m_typeMissionRead
        , qvgApp->mavLink(), m_dedicatedLink->getMavlinkChannel(), m_vehicle->sysID());
    m_vehicle->sendMessage(message);
...
void MissionManager::_handleMissionAck(const mavlink_message_t& message)
{
    ...
    case AckNone:
        ...
        _sendError(VehicleError, tr("Mission failure respond--%1").arg(strTmp));//"任务异常回应--%1"
        break;
    case AckMissionCount:
        // MISSION_COUNT message expected
        _sendError(VehicleError, tr("Device failure respond--%1").arg(strTmp));//"设备返回错误--%1"
        _finishTransaction(false);
        break;
    case AckMissionItem:
        // MISSION_ITEM expected
        _sendError(VehicleError, tr("Device failure respond--%1").arg(strTmp));
        _finishTransaction(false);
        break;
    case AckMissionRequest:
        // MISSION_REQUEST is expected, or MISSION_ACK to end sequence
        if (missionAck.type == MAV_MISSION_ACCEPTED)
        {
            ...
            {
                _sendError(MissingRequestsError, tr("Mission upload fail,read %1 break").arg(m_itemIndicesToWrite.count()));//"上传航线失败，读取第%1点中断"
                _finishTransaction(false);
            }
        }
        else
    ...
    case AckMissionClearAll:
        // MISSION_ACK expected
        if (missionAck.type != MAV_MISSION_ACCEPTED)
            _sendError(VehicleError, tr("Device failure respond--%1,delete fail!").arg(strTmp));//"设备返回错误--%1，删除失败！"
    ...
    case AckGuidedItem:
        ...
        if (missionAck.type == MAV_MISSION_ACCEPTED) {
            ...
            _sendError(VehicleError, tr("Device failure respond--%1,guide item reject!").arg(strTmp));//"设备返回错误--%1，不接收引导Item！"
            _finishTransaction(false);
        }
        break;
    }
...
void MissionManager::handleMessage(const mavlink_message_t& message)
{
...
QString MissionManager::missionResultToString(MAV_MISSION_RESULT result)
{
    ...
    case MAV_MISSION_ACCEPTED:
        resultString = tr("Mission accepted");//"任务接收"
        break;
    case MAV_MISSION_ERROR:
        resultString = tr("Mission error");//"任务错误"
        break;
    case MAV_MISSION_UNSUPPORTED_FRAME:
        resultString = tr("Mission frame un-support");//"任务FRAME类型不支持"
        break;
    case MAV_MISSION_UNSUPPORTED:
        resultString = tr("Mission command un-support");//"任务COMMAND类型不支持"
        break;
    case MAV_MISSION_NO_SPACE:
        resultString = tr("No storage");//"没有存储空间"
        break;
    case MAV_MISSION_INVALID:
        resultString = tr("Parameter invalid");//有参数无效
        break;
    case MAV_MISSION_INVALID_PARAM1:
        resultString = tr("PARAM1 invalid");//"PARAM1无效"
        break;
    case MAV_MISSION_INVALID_PARAM2:
        resultString = tr("PARAM2 invalid");//"PARAM2无效"
        break;
    case MAV_MISSION_INVALID_PARAM3:
        resultString = tr("PARAM3 invalid");//"PARAM3无效"
        break;
    case MAV_MISSION_INVALID_PARAM4:
        resultString = tr("PARAM4 invalid");//"PARAM4无效"
        break;
    case MAV_MISSION_INVALID_PARAM5_X:
        resultString = tr("PARAM5 invalid");//"PARAM5无效"
        break;
    case MAV_MISSION_INVALID_PARAM6_Y:
        resultString = tr("PARAM6 invalid");//"PARAM6无效"
        break;
    case MAV_MISSION_INVALID_PARAM7:
        resultString = tr("PARAM7 invalid");//"PARAM7无效"
        break;
    case MAV_MISSION_INVALID_SEQUENCE:
        resultString = tr("Sequence out of range");//任务点序列超限
        break;
    case MAV_MISSION_DENIED:
        resultString = tr("Device reject mission");//设备禁止接收任务
        break;
    default:
        resultString = tr("Station error");//"地面站内部错误"
    }
