...
Page {
    ...
    Label {
        ...
        text:       qsTr("Register user")//qsTr("注册用户")
    ...
    Grid
    {
        ...
        Label{
            text: qsTr("Confirm code:")//qsTr("验证码:")
            font: vgMainPage.font()
            verticalAlignment : Text.AlignVCenter|Text.Left
        }
        VGTextField {
        ...
        Label{
            text:   qsTr("Password longer than 3")//qsTr("密码大于3个字节")
            color:  "#0000E0"
        }
        Label{
            text: qsTr("Confirm password:")//qsTr("确认密码:")
            font: vgMainPage.font()
            verticalAlignment : Text.AlignVCenter|Text.Left
        }
        VGTextField {
    ...
    Button{
        ...
        text: qsTr("Register")//qsTr("注册")
        style: VGButtonStyle {}
        //radius: 10
        onClicked: {
            if(password.text!==pswdCheck.text) {
                lbCheck.text = qsTr("Difference of 2 passwords")//qsTr("密码不一致")
                return
            }
            netManager.registerUser(account.text, password.text, check.text)
            if(check.text!==netManager.check)
                return
        }
    }
    Connections{
    ...
    Connections{
        ...
        onSigShowPage: {
            ...
            if (root.visible) {
                ...
                vgMainPage.headerName = qsTr("Register")
            }
        }
        onSigBack: {
            if (page === root)
