...
Rectangle{
    ...
    Rectangle{
        ...
        Text {
            ...
            text: qsTr("Click boundary point, click button of orientation move")//"在地图上单击边界点，点击方向移动"
            color: "#454545"
        ...
        Text {
            ...
            function getAdjustText() {
                var strText = ""
                if (vertiOffset!==0)
                    strText = qsTr("To %1 move %2 m").arg(vertiOffset>0?qsTr("down"):qsTr("up")).arg(0.2*vertiOffset*(vertiOffset>0?1:-1))//"向%1偏移%2米"，下，上
                if (horizOffset!==0) {
                    if (strText.length)
                        strText += ","
                    strText += qsTr("To %1 move %2 m").arg(horizOffset>0?qsTr("right"):qsTr("left")).arg(0.2*horizOffset*(horizOffset>0?1:-1))//"向%1偏移%2米"，右，左
                }
                return strText
            }
        }
