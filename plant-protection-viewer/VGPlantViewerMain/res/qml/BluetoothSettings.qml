...
Rectangle
{
    ...
    Rectangle {
        ...
        width: parent.width*2/(parent.width>parent.height ? 4 : 3)
        height: parent.height*2/(parent.width>parent.height ? 3 : 4)
        color:  "white"
        radius: 6
        MouseArea{
            anchors.fill:   parent
        ...
        Text {
            ...
            text:   qsTr("Bluetooth link")//qsTr("蓝牙设备连接")
            font:   vgMainPage.font(true)
            color:  "#01113E"
        }
        VGCheckBox {
            anchors {verticalCenter: txtHeader.verticalCenter; right: parent.right; rightMargin: 5}
            text:    qsTr("Auto link")///"自连"
            checked: linkManager.bleAutotLink
            onCheckedChanged:   linkManager.bleAutotLink = checked
        }
        Rectangle{
            id: sperater
            height: 1
        ...
        Text {
            text:       qsTr("Bluetooth is unvalible")//qsTr("蓝牙设备不可用")
            visible:    !linkManager.bleAvailable
            anchors.centerIn: parent
        }
