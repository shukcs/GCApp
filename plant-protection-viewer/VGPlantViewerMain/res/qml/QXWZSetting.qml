...
Rectangle {
    ...
    color:                  "#F5F5F5"
    anchors.fill:           parent
    property var suveyUav:   plantManager.surveyUav
    property bool bConnect:  suveyUav && suveyUav.status===VGPlantInformation.Connected
    MouseArea{
        anchors.fill:                   parent
        onClicked:  {}
        onWheel:    {}
    ...
    Rectangle {
        ...
        Label{
            ...
            text:       qsTr("QX setting")//qsTr("千寻定位设置")
    ...
    VGTextButton{
        ...
        strText:        testPage.visible ? qsTr("Hide"):qsTr("Test")//qsTr("隐藏"):qsTr("测试")
    ...
    Rectangle {
        ...
        function setSurvey() {
            ...
            var page = null
            if (tp === MapAbstractItem.Survey_BanLv)
        ...
        VGComboBox{
            ...
            model:   [qsTr("Beidu survey"), qsTr("Chongqing RTK survey"), qsTr("Plant survey")]//"北斗伴侣踩点"，"重庆单频RTK测绘站"，"飞机踩点"
        }
        VGToolButton{
            anchors {verticalCenter: cbbSurveyType.verticalCenter; right: parent.right; rightMargin: 20}
            visible: cbbSurveyType.currentIndex !== 2
            iconName: "bluetooth"
        ...
        VGToolButton{
            ...
            visible: cbbSurveyType.currentIndex === 2
            iconName: "craft"
            onBtnClicked:   vgMainPage.curQmlPage = comUavSelect.createObject(root)
        }
        Label{
            id:lbLat
        ...
        Label{
            ...
            text:       cbbSurveyType.currentIndex === 2 ? (bConnect? suveyUav.lat.toFixed(8):qsTr("N/A"))
                                                : (surveyMonitor.connected ? surveyMonitor.lat.toFixed(8):qsTr("N/A"))
            font:       vgMainPage.font()
        }
        Label{
            id:lbLon
        ...
        Label{
            ...
            text:       cbbSurveyType.currentIndex === 2 ? (bConnect? suveyUav.lon.toFixed(8):qsTr("N/A"))
                                                         : surveyMonitor.connected ? surveyMonitor.lon.toFixed(8):qsTr("N/A")
            font:       vgMainPage.font()
        }
        Label{
            id:lbAlt
            anchors {left: lbLat.left; top:lbLon.bottom; topMargin: 6}
            text:       qsTr("Height:")//qsTr("高度：")
            font:       vgMainPage.font()
        }
        Label{
            anchors {left: cbbSurveyType.horizontalCenter; top: lbAlt.top}
            text:       cbbSurveyType.currentIndex === 2 ? (bConnect? suveyUav.heigth.toFixed(3):qsTr("N/A"))
                                                         : surveyMonitor.connected ? surveyMonitor.relativeAltitude.toFixed(4):qsTr("N/A")
            font:       vgMainPage.font()
        }
        Label{
            id:lbLocation
            anchors {left: lbLat.left; top:lbAlt.bottom; topMargin: 6}
            text:       qsTr("Position:")//qsTr("定位：")
            font:       vgMainPage.font()
        }
        Label{
            anchors {left: cbbSurveyType.horizontalCenter; top: lbLocation.top}
            text:       cbbSurveyType.currentIndex === 2 ? suveyUav.posTip
                                                         : surveyMonitor.posTip
            font:       vgMainPage.font()
        }
    }
    Connections{
