...
Rectangle {
    ...
    property alias  strLength:      txtOther.text
    property bool  bInit:           false
    property bool  bEditFirts:      true
    signal finishValue(string strKey, double valLit, double valBig, bool exit)
    function initial(max, little, bigger) {
        bInit = false
        rctBg.from = 1
        rctBg.to = max
        rctBg.stepSize = 1
    ...
    Rectangle{
        ...
        RangeSlider {
            ...
            first.onValueChanged: {
                if (bEditFirts)
                    root.finishValue(strHeader, first.value, second.value, false)
            }
