...
uchar VGBDLocation::_prcsPackInit(const QByteArray &arr)
{
    ...
    if ((m_lstFramStat.GetFirstFrom(false) >= 0 && !_isTimeout()))
        return 0;
    uint8_t nFram = uint8_t(arr[3]);
    m_lstFramStat.SetSize(nFram);
    m_content.resize(PACKFRAM_SIZE * nFram);
    m_bIdle = false;
...
void VGBDLocation::_parseNmea(const QString &str)
{
    foreach(const QString &strItr, str.split("\r\n", QString::SkipEmptyParts))
    {
        ...
        QString header = strLs.size() > 0 ? strLs.first():QString();
        if (header == "$GPGGA" || header=="$GNGGA")
        {
            if (_parPos(strLs) > 0)
...
{
    qvgApp->surveyMonitor()->SetConnected(b, VGSurveyMonitor::Dev_Baidu);
    if (b == m_bConnect)
        return;
