...
void AudioWorker::_addAudioItem(const QString &key, const QStringList &params)
{
    if (params.count() == 0)
    {
        if (!m_queAudioMed.contains(key))
            m_queAudioMed.append(key);
        return;
    }

    if (AudioItem *item = _getAudioItem(key))
        item->second = params;
    else
        m_queAudioLoop.append(AudioItem(key, params));
}
...
void AudioWorker::_wait()
{
	m_queAudioLoop.clear();
	m_queAudioMed.clear();
	QTimer::singleShot(20000, this, &AudioWorker::onTimer);
}
