...
void VGQXManager::onReparse()
{
    if (QDateTime::currentMSecsSinceEpoch()-m_tmLastRTCM>100)
        _parseRtcm(true);
}
...
void VGQXManager::_parseRtcm(bool bSend)
{
    const QByteArray &value = m_arrayRecv;
    QList<uint16_t> idxs;
    QList<QByteArray> rtcms;
    for (int i = 0; i < m_arrayRecv.size() - 3; ++i)
    {
        if (uint8_t(m_arrayRecv[i]) == FRAME_HEADER)
        {
            uint16_t len = (VGGlobalFunc::bytesToUShort(m_arrayRecv.mid(i + 1, 2)) & 0x03ff) + 6;
            if (len <= value.size()-i && len<512)
            {
                uint16_t type = ((VGGlobalFunc::bytesToUShort(m_arrayRecv.mid(i+3, 2))>>4) & 0x0fff);
                SetRtcm(true);
                ...
                if (bSend || bRTCMPack)
                {
                    int lenTmp = bRTCMPack ? i : i + len;
                    emit rtcmRead(idxs, rtcms);
                    m_arrayRecv.remove(0, lenTmp);
                    return;
                }
    ...
    if(idxs.size() > 0)
        QTimer::singleShot(100, this, &VGQXManager::onReparse);
}
