    ...
    , m_controller(NULL)
{
    ...
    {
#ifdef __mobile__
        QString path = "assets:/lng/VigaApp.qm";
#else
        QString path = applicationDirPath() + "/VigaApp.qm";
#endif
...
VGController * VGApplication::controller() const
{
    return m_controller;
}

VGFriendsManager *VGApplication::friendsManager() const
{
    if (m_pMainUIInterface)
        return m_pMainUIInterface->friendsManager();
    return NULL;
}
...
void VGApplication::_initMember()
{
    ...
    if (m_controller = new VGController(this))
        connect(plantManager(), &VGPlantManager::paramChanged,
            m_controller, &VGController::OnParamChanged);
}
